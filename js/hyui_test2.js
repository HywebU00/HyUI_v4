...
class FontSize {
  constructor(obj) {
    this.name = document.querySelectorAll(`${obj.el}`); // --按鈕列表名稱
    this.control = document.querySelector(`${obj.control}`); // --控制的對象名稱
  }
  // 點擊文字大小按鈕
  clickFontBtn() {
    this.name.forEach((i) => {
...
/////////////// FatFooter ///////////
/*-----------------------------------*/
class FatFooter extends SliderMethod {
  constructor(obj) {
    super();
  ...
  toggleFatFooter(_that) {
    let _navUl = _that.name.parentNode.querySelectorAll('nav ul li ul');
    _navUl.forEach((i) => {
      if (i.offsetHeight !== 0) {
        super.slideUp(i, 300);
...
$(function () {
  ...
  class ScrollToTop {
    constructor(obj) {
      this.name = document.querySelector(`${obj.el}`); //obj.name || null; //監聽的對象ㄋ
    }
    scrollTop() {
      window.scrollTo({
        top: 0,
